...
truffleHog --help > /dev/null || { echo -e "${FAILED_MESSAGE} truffleHog is not installed. Please install via https://github.com/trufflesecurity/truffleHog" ; exit 1; }
...
truffleHog --regex --since_commit $(git rev-parse origin/HEAD) --branch $(git rev-parse --abbrev-ref HEAD) file://${DEFRA_WORKSPACE}/${REPO_NAME}
exit_code=$?
