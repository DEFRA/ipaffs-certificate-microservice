...
def call(body) {
    ...
    pipeline {
        ...
        stages {
            ...
            stage('Daily pipeline check') {
                when {
                    allOf{
                        branch 'hotfix/IMTA-11039-daily-pipeline-check'
                        expression { !params.IS_DAILY_PIPELINE }
                    }
                }
                steps {
                    error('This build can only be triggered by Daily_Hotfix_Pipeline_Test job.')
                }
            }
            ...
            stage('Code Secret Scanning'){
                ...
                steps {
                    build job: 'Secret-Scanning-Local',
                        parameters: [
            ...
            stage('Compile') {
                ...
                post {
                    success {
                        echo "Successfully compiled Java microservice"
                    }
                }
            }
                ...
                post {
                    always {
                        script {
                            ...
                            if (fileExists('integration/target/cucumber/')) {
            ...
            stage('Selenium Tests') {
                when {
                    expression {
                        return env.BRANCH_NAME.startsWith("hotfix") && !shouldSkipStage("${SKIP_SELENIUM}")
                    }
                }
                steps {
